...
// ═══════════════════════════════════════════════════════════════
// CARDIO SCREEN (separate component to isolate timer hooks)
// ═══════════════════════════════════════════════════════════════
...
export default function App() {
      ...
      <div style={{ minHeight: '100vh', background: BG, color: '#fff', fontFamily: "'Tajawal', sans-serif", direction: dir }}>
        ...
        <div style={{ padding: '20px 16px 100px' }}>
          ...
          {data.sessions.length > 0 && (
            <>
              <div style={{ fontSize: 13, fontWeight: 700, color: MUTED, marginBottom: 10 }}>{t.history}</div>
              ...
              {data.sessions.length > 4 && (
                <Btn variant="ghost" onClick={() => go('history')} style={{ color: ACCENT, fontSize: 14, padding: '10px' }}>
                  {t.history} →
                </Btn>
              )}
            </>
  ...
  // ─── WORKOUT (strength / hypertrophy days) ───
                  ...
                  {Array.from({ length: ex.sets }).map((_, si) => {
                    ...
                    return (
                      <button key={si} onClick={() => {
                        if (!isDone) patchEx(i, { done: si + 1 });
                        else if (si === ex.done - 1) patchEx(i, { done: si });
                      }} style={{
